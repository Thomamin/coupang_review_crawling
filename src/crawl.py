    ...
    def fetch(self,url:str,session)-> List[Dict[str,Union[str,int]]]:
        ...
        with session.get(url=url,headers=self.__headers) as response :
            ...
            for idx in range(article_lenth):
                ...
                # 날짜
                date_w = articles[idx].select_one('div.sdp-review__article__list__info__product-info__reg-date')
                if date_w == None:
                    date_w = '0000.00.00'
                else:
                    date_w = date_w.text.strip()
...
class OpenPyXL:
    ...
    def save_file()-> None:
        ...
        wb = Workbook()
        ws = wb.active
        ws.append(['날짜','상품명','구매자 이름','구매자 평점','리뷰 제목','리뷰 내용','맛 만족도'])

        row = 2

        for x in results:
            for result in x :
                ws[f'A{row}'] = result['date_w']
                ws[f'B{row}'] = result['prod_name']
                ws[f'C{row}'] = result['user_name']
                ws[f'D{row}'] = result['rating']
                ws[f'E{row}'] = result['headline']
                ws[f'F{row}'] = result['review_content']
